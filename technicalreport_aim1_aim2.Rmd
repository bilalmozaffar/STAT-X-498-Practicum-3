...
```{r}
library(nlme)
m1.amb = lme(lActivity ~ lAltitude + lTrout + Caloric + Water.Excess + Amoeba + Month + Site + lAltitude:Otter + lTrout:Otter + Amoeba:Otter, random = ~1+Month|Otter, otters.amoeba)
summary(m1.amb)
```
...
AIC(update(m5.amb.ml, .~.-Amoeba)) #The lowest AIC is reached, which is the same as in m5.amb.ml, keep m5.amb.ml as the final model for selection of fixed effects
...
```{r}
m1.wt = lmer(lActivity ~ lAltitude + lTrout + Caloric + Water.Excess + Water + Month + (1|Site), otters.water)
summary(m1.wt)
```
